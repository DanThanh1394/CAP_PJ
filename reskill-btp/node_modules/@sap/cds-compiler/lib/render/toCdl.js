...
/**
 * Render a CSN expression object to CDL-like text.
 *
 * @param {object} expr CSN expression node
 * @param {CdlOptions} [options]
 * @param {object} [msg] Message functions
 * @returns {string}
 */
function expressionToCdlString( expr, options = {}, msg = null ) {
  const model = { $version: '2.0', definitions: Object.create(null) };
  const messageFunctions = msg || makeMessageFunction(model, options, 'to-cdl');
  const renderer = new CsnToCdl(model, options, messageFunctions);
  const env = createEnv({ path: [] });
  env.nameEnvStack.setRootScope(new DefinitionPathTree(), model);
  return renderer.exprRenderer.renderExpr(expr, env);
}

module.exports = {
  csnToCdl,
  expressionToCdlString,
  smartId: apiSmartId,
